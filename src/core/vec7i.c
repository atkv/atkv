...
at_vec7i_normalize(AtVec7i* vec){
  double magnitude = at_vec7i_get_magnitude(vec);
  at_vec7i_multiply_scalar(vec,1.0/magnitude);
}
double
at_vec7i_get_magnitude(AtVec7i* vec){
  return sqrt((double)at_vec7i_dot(vec,vec));
}


void
at_vec7i_negate(AtVec7i* vec){
  uint8_t i;
  for(i = 0; i < 7; i++)
